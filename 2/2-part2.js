...
const getPossibleGames = (values) => {
  return values
    .map((value) => {
      return isPossibleForValue(value);
    })
    .filter((el) => el !== undefined)
    .reduce((acc, current) => acc + current, 0);
};

const isPossibleForValue = (value) => {
  // const red = 12;
  // const green = 13;
  // const blue = 14;
  let reds = [];
  ...
  // const game = value.split(":")[0].slice(5);
  const games = value.split(":")[1].split(";");
  games.forEach((game) => {
    game.split(",").forEach((element) => {
      if (element.trim().includes("red")) {
        reds.push(parseInt(element.replace("red", "").trim()));
      } else if (element.trim().includes("green")) {
        greens.push(parseInt(element.replace("green", "").trim()));
      } else if (element.trim().includes("blue")) {
        blues.push(parseInt(element.replace("blue", "").trim()));
      }
    });
  });
...
console.log(getPossibleGames(values));
