...
const getAsteriskIndexes = (input: string) => {
  return [...input.matchAll(/\*/g)].map((asterisk) => {
    return asterisk.index;
  });
};

const getAdjacentNumbersForAsterisk = (
    ...
    .map((line, lineIndex) => {
      ...
      if (asterisksAt.length > 0) {
        const adjacentNumbersForAsterisk = asterisksAt.map((asteriskIndex) => {
          ...
          const adjacentNumbers = getAdjacentNumbersForAsterisk(
            asteriskIndex!,
            allSurroundingNumbers
          );
