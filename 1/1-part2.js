...
const replaceWordsToDigits = (fullString) => {
  ...
  // console.log(words)
  const resultWordNumbers = words
    .map((word) => {
      console.log(
        `${fullString.indexOf(word.name)}, ${
          word.name
        }, ${fullString.lastIndexOf(word.name)}`
      );
      if (fullString.indexOf(word.name) > -1) {
        if (
          fullString.indexOf(word.name) === fullString.lastIndexOf(word.name)
        ) {
          return { index: fullString.indexOf(word.name), value: word.value };
        } else {
          return [
  ...
  const resultNumbers = fullString
    .split("")
    .map((char, index) => {
      if (Number(char)) {
        return { index, value: char };
      }
    })
    .filter((value) => value != undefined);

  const result = resultWordNumbers.flat().concat(resultNumbers);
  ...
  return sorted.map((value) => value.value).join("");
};

const calibration = (values) => {
  const sanitizedValues = values.map((value) => replaceWordsToDigits(value));
  const result = sanitizedValues.map((value) => {
    const digits = value.split("").filter((char) => Number(char));
    const firstDigit = digits[0];
    const lastDigit = digits.slice(-1)[0];

    return firstDigit + lastDigit;
    // return updatedDigits.reduce((a, b) => a + b)
  });
  console.log(result);
  return result.map((el) => parseInt(el)).reduce((x, y) => x + y);
};

// const values = ["rgvxddcstwo82one67two"]
// const values = ['three313eight2zcjnnmtnh']
console.log(calibration(values));
// console.log(replaceWordsToDigits("zzeightwobbthreeaa"));
// console.log("heyyyyy")
